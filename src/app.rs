...
impl App {
    ...
    fn move_selection(&mut self, delta: i32) {
        let (state, len) = match self.focus {
            Pane::Library => (&mut self.library_state, self.library.len()),
        ...
        match self.focus {
            Pane::Library => {
                if let Some(i) = self.library_state.selected() {
                    if let Some(t) = self.library.get(i).cloned() {
                        self.queue.push(t);
        ...
        if self.focus == Pane::Library {
            if let Some(i) = self.library_state.selected() {
                if let Some(t) = self.library.get(i).cloned() {
                    ...
                    self.status = format!("Queued: {}", t.title);
    ...
    fn play_current(&mut self) {
        if let Some(i) = self.queue_index {
            if let Some(t) = self.queue.get(i).cloned() {
                match self.player.play(&t.path) {
                    Ok(_) => self.status = format!("Playing: {}", t.title),
    ...
    fn advance(&mut self) {
        if self.config.playback.repeat {
            self.play_current();
            return;
        }
        if let Some(i) = self.queue_index {
            if i + 1 < self.queue.len() {
                self.queue_index = Some(i + 1);
        ...
        for entry in WalkDir::new(dir).into_iter().filter_map(|e| e.ok()) {
            ...
            if let Some(e) = ext {
                if AUDIO_EXTS.contains(&e.as_str()) {
                    out.push(Track::from_path(path.to_path_buf()));
                }
            }
        }
    }
